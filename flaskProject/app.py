...
@app.route('/crud_instructor', methods=['GET', 'POST'])
def crud_instructor():
    msg = ''

    cursor = db.cursor()

    # Load all instructors for dropdowns & table
    cursor.execute("""
        SELECT instructor_id, first_name, middle_name, last_name, salary
        FROM instructor
    ...
    if request.method == 'POST':
        ...
        if action == 'create':
            ...
            try:
                ...
                cursor.execute("""
                    ...
                    VALUES (%s, %s, %s, %s, %s)
                """, (id, fname, mname, lname, salary))
                db.commit()
                msg = "Instructor created successfully!"
            except Exception as e:
                db.rollback()
                msg = f"Error creating instructor: {e}"
            finally:
                cursor.close()

        # ---------- READ ----------
        elif action == 'read':
            ins_id = request.form['instructor_id']
            ...
            return render_template(
                'actions/admin/crud_instructor.html',
                msg=msg,
        ...
        # ---------- UPDATE ----------
            ...
            new_mname  = request.form.get('new_mname')
            ...
            try:
                ...
                if updates:
                    sql = "UPDATE instructor SET " + ", ".join(updates) + " WHERE instructor_id = %s"
                    params.append(instructor_id)
                    cursor.execute(sql, params)
                    db.commit()
                    msg = "Instructor updated successfully!"
                else:
                    msg = "No fields entered."

            except Exception as e:
                db.rollback()
                msg = f"Error updating instructor: {e}"
            finally:
                cursor.close()
        ...
        # Reload instructor list after any POST
        cursor = db.cursor()
        cursor.execute("""
            SELECT instructor_id, first_name, middle_name, last_name, salary
            FROM instructor

...
def crud_course():
    ...
    if request.method == "POST"  and 'Cid' in request.form: #we creating out here
        ...
        cursor = db.cursor()
        try:
            cursor.execute("SELECT * FROM prereq WHERE course_id = %s", (course_id,))
            test1=cursor.fetchone()

            cursor.execute("SELECT * FROM has_course WHERE course_id = %s", (course_id,))
            test2=cursor.fetchone()

            cursor.execute("SELECT * FROM has_sections WHERE course_id = %s", (course_id,))
            test3=cursor.fetchone()

            cursor.execute("SELECT * FROM prereq WHERE prereq_id = %s", (course_id,))
            test4=cursor.fetchone()
            ...
            if not test1 and not test2 and not test3 and not test4:
                ...
                msg = "Could not delete- has dependencies."
...
def crud_section():
    ...
    if request.method == "POST"  and 'Cid' in request.form: #we creating out here
        ...
        cursor = db.cursor()
        try:
            cursor.execute("SELECT * FROM held_in WHERE section_id = %s", (section_id,))
            test1=cursor.fetchone()

            cursor.execute("SELECT * FROM held_during WHERE section_id = %s", (section_id,))
            test2=cursor.fetchone()

            cursor.execute("SELECT * FROM has_sections WHERE section_id = %s", (section_id,))
            test3=cursor.fetchone()

            cursor.execute("SELECT * FROM is_offered WHERE section_id = %s", (section_id,))
            test4=cursor.fetchone()
            
            cursor.execute("SELECT * FROM teaches WHERE section_id = %s", (section_id,))
            test5=cursor.fetchone()
            
            if not test1 and not test2 and not test3 and not test4 and not test5:
...
def crud_classroom():
    ...
    if request.method == "POST"  and 'Cid' in request.form: #we creating out here
        ...
        cursor = db.cursor()
        try:
            cursor.execute("SELECT * FROM held_in WHERE room_id = %s", (room_id,))
...
def crud_department():
    ...
    if request.method == "POST"  and 'Cid' in request.form: #we creating out here
        ...
        cursor = db.cursor()
        try:
            cursor.execute("SELECT * FROM employed WHERE dept_id = %s", (dept_id,))
            test1=cursor.fetchone()
            
            cursor.execute("SELECT * FROM has_course WHERE dept_id = %s", (dept_id,))
            test2=cursor.fetchone()
            
            cursor.execute("SELECT * FROM under WHERE dept_id = %s", (dept_id,))
            test3=cursor.fetchone()
...
def crud_timeslot():
    ...
    if request.method == "POST"  and 'Cid' in request.form: #we creating out here
        ...
        cursor = db.cursor()
        try:
            cursor.execute("SELECT * FROM held_during WHERE time_slot_id = %s", (time_slot_id,))
            test=cursor.fetchone()
...
def crud_instructor():
    ...
    if request.method == "POST"  and 'Cid' in request.form: #we creating out here
        ...
        cursor = db.cursor()
        try:
            cursor.execute("SELECT * FROM employed WHERE instructor_id = %s", (instructor_id,))
            test1=cursor.fetchone()

            cursor.execute("SELECT * FROM advisor WHERE instructor_id = %s", (instructor_id,))
            test2=cursor.fetchone()
...
def crud_student():
    ...
    if request.method == "POST"  and 'Cid' in request.form: #we creating out here
        ...
        cursor = db.cursor()
        try:
            cursor.execute("SELECT * FROM declared WHERE student_id = %s", (student_id,))
            test1=cursor.fetchall()

            cursor.execute("SELECT * FROM advisor WHERE student_id = %s", (student_id,))
            test2=cursor.fetchall()

            cursor.execute("SELECT * FROM enrolled WHERE student_id = %s", (student_id,))
...
@app.route('/average_sem', methods=['GET', 'POST'])
def average_sem():
    msg = ''
    results = None
    courses = []
    ...
    try:
        ...
        print("average_sem (load courses) ERROR:", e)
        msg = f"Error loading courses: {e}"
        return render_template(
            'actions/instructor/average_sem.html',
            msg=msg,
            courses=courses,
            results=results
        )
    ...
    if request.method == 'POST':
        ...
        end_semester   = request.form.get('end_semester')
        end_year       = request.form.get('end_year')

        if not all([course_id, start_semester, start_year, end_semester, end_year]):
            msg = "Please select a course and fill in all semester fields."
        else:
            try:
                ...
                query = """
                    ...
                    FROM course c
                    JOIN has_sections hs ON c.course_id = hs.course_id
                    JOIN section      s  ON s.section_id = hs.section_id
                    JOIN is_offered   io ON io.section_id = s.section_id
                    JOIN enrollment   e  ON e.enrollment_id = io.enrollment_id
                    WHERE c.course_id = %s
                      AND (s.year * 10 + FIELD(s.semester, 'Spring','Summer','Fall','Winter'))
                          BETWEEN
                          (%s * 10 + FIELD(%s, 'Spring','Summer','Fall','Winter'))
                          AND
                          (%s * 10 + FIELD(%s, 'Spring','Summer','Fall','Winter'))
                ...
                if row:
                    ...
                    msg = "No graded enrollments for that class in the selected range."
            except Exception as e:
                print("average_sem (compute avg) ERROR:", e)
                msg = f"Error fetching averages: {e}"
...
def rankings():
    ...
    if request.method == 'POST':
        ...
        if not semester or not year:
            msg = "Please select both a semester and a year."
        else:
            try:
                year = int(year)
                cursor = db.cursor()
                ...
                if not results:
                    msg = "No graded enrollments found for that semester."

            except Exception as e:
                print("rankings ERROR:", e)
                msg = f"Error fetching rankings: {e}"

    return render_template(
        'actions/instructor/rankings.html',
        msg=msg,
        results=results,
        semester=semester,
        year=year
...
@app.route('/all_stud_dept', methods=['GET'])
def all_stud_dept():
    msg = ''
    results = []
    ...
    try:
        ...
        # Count ALL students per department (past + current)
        query = """
            SELECT
                d.dept_id,
                d.dept_name,
        ...
        print("all_stud_dept ERROR:", e)
        msg = f"Error fetching totals: {e}"

    return render_template(
        'actions/instructor/all_stud_dept.html',
        msg=msg,
        results=results
    )

@app.route('/current_stud_dept', methods=['GET'])
def current_stud_dept():
    msg = ''
    results = []
    ...
    try:
        ...
        query = """
            SELECT
                d.dept_id,
                d.dept_name,
                COUNT(DISTINCT CASE WHEN e.grade IS NULL THEN s.student_id END)
                    AS current_students
            FROM department d
            LEFT JOIN student  s  ON s.dept_name = d.dept_name
            LEFT JOIN enrolled en ON en.student_id = s.student_id
            LEFT JOIN enrollment e ON e.enrollment_id = en.enrollment_id
            GROUP BY d.dept_id, d.dept_name
            ORDER BY d.dept_id;
        """
        ...
        print("current_stud_dept ERROR:", e)
        msg = f"Error fetching current totals: {e}"

    return render_template(
        'actions/instructor/current_stud_dept.html',
        msg=msg,
        results=results
    )
